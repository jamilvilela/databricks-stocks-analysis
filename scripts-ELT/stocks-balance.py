...
# MAGIC %md
# MAGIC ### Annual and Quarterly Companies' Balance 
...
log['entity_name'] = params['TABLE_BALANCE']
log['start_time'] = start_time
log['source_type'] = 'API'
log['source_path'] = 'yfinance.ticker.balance_sheet'
log['source_entity'] = 'balance_sheet'
...
for item in ticker_list:
    try:
        ...
        df_quarter = ticker.quarterly_balance_sheet
...
balance_df = spark.createDataFrame(df, schema_bronze)
...
util.merge_delta_table(balance_df, 'BRONZE', 'BALANCE', args)
...
log = util.write_log(log, 'BRONZE', 'BALANCE')
...
schema_silver = StructType([ 
                    StructField('balance_period',StringType(), nullable=False), 
                    StructField('ticker',StringType(), nullable=False), 
                    StructField('balance_description',StringType(), nullable=False), 
                    StructField('balance_report_date',DateType(), nullable=False), 
                    StructField('balance_value',DoubleType(), nullable=False), 
                    StructField('ymd',StringType(), nullable=True)
                ])
...
util.merge_delta_table(balance_df_new, 'SILVER', 'BALANCE', args)
...
util.write_log(log, 'GOLD', 'BALANCE')
